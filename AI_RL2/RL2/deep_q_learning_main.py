...
def act_loop(env, agent, num_episodes):
    for episode in range(num_episodes):
        observation = env.reset()   # env gives one observation
        if timeHorizon:
            observation = np.append(observation,1)
        agent.reset_episode(observation)  # reset 
        ...
        while True:
            ...
            action = agent.select_action(observation)   # action -> old network
            ...
            if done:
                ...
                agent.report()
                break
        agent.Q_target.load_state_dict(qn.state_dict())  # Q_target every epsoid update; qn every step update
