...
class ReplayMemory(object):
    ...
    def __init__(self, size, shape_o = (9, )):
        ...
        self.action_memory = np.zeros(self.mem_size, dtype = np.int64)   # later translate into pandas
...
class QNet(nn.Module):
    ...
    def max_Q_value(self, observation, batch_size=1):
        ...
        Qs = self.forward(observation)  # <- this should feed in the input, only obs, output : Q_value of action; q real - q estimate
    ...
    def single_Q_update(self, prev_observation, action, observation, reward, done, fu_val = 0):
        ...
        #else:
        #    future_val = self.max_Q_value(t_observation)  ##<<- this evaluates the QNet     #based on theta- , 
        ...
        t_predict = self.get_Q(t_prev_obs, action)  ##<<- this evaluates the QNet  Q_eval
        ...
        td = target - t_predict.detach().numpy() # q_target
    ...
    def batch_Q_update(self, obs, actions, next_obs, rewards, dones, qtarget = 0):
        ...
        targets = qtarget 
        self.zero_grad()
        q_pred = self.get_Q(obs, actions, batch_size)  #q_eval
        loss = self.loss_fn(q_pred, torch.tensor(targets, dtype=torch.float))   # update the eval network, q_eval == q_pred; q_target is the target net get 
        if DEBUG:
...
class QNet_MLP(QNet):
    ...
    def init_network(self, obs_shape, num_a):
        num_in = np.prod(obs_shape)   # only input the state
...
class QLearner(object):
    def __init__(self, env, q_function, target_qn = None, discount=DEFAULT_DISCOUNT, rm_size=RMSIZE):
        self.env = env
        self.Q = q_function
        self.Q_target = target_qn;
    ...
    def reset_episode(self, initial_obs):  #
        self.last_obs = initial_obs # last  =this
    ...
    def process_experience(self, action, observation, reward, done):
        ...
        #if self.Q_target :	
        #       self.Q_target.single_Q_update(prev_obs, action, observation, reward, done)  #for the next state
        self.last_obs = observation
        
        t_observation = self.Q_target.obs_to_tensor(observation)
        if done:
           future_val = 0  # XXX<- needs to be a 0-tensor?
        else:
           future_val = self.Q_target.max_Q_value(t_observation)  ##<<- this evaluates the QNet     #based on theta- , 
        ...
        if self.tot_stages > 10 * self.batch_size:                    
           ...
           qtarget_next_obs = self.Q_target.max_Q_value(states_, len(terminal))
           qtarget_fut_values = self.Q_target.discount * qtarget_next_obs * (1 - terminal)
